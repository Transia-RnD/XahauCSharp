...
namespace Xahau.Tests.Wallet.Tests
{
    ...
    public class TestUSigner
    {
        ...
        XahauWallet verifyWallet = new XahauWallet(publicKey, privateKey, address);
        ...
        {
            XahauWallet secpWallet = XahauWallet.FromSeed("snGHNrPbHrdUcszeuDEigMdC1Lyyd");
        ...
        {
            XahauWallet edWallet = XahauWallet.FromSeed("sEdSuqBPSQaood2DmNYVkwWTn1oQTj2");
            string channelId = "5DB01B7FFED6B67E6B0414DED11E051D2EE2B7619CE0EAA6286D67A3A4D5BDB3";
            string amount = "1000000";
            Assert.AreEqual(
        ...
        {
            SignatureResult signedTx = verifyWallet.Sign(tx);
            Assert.IsTrue(Signer.VerifySignature(XahauBinaryCodec.Decode(signedTx.TxBlob).ToObject<Dictionary<string, dynamic>>()));
        }
        ...
        {
            SignatureResult signedTx = verifyWallet.Sign(tx);
            Dictionary<string, dynamic> decodedTx = XahauBinaryCodec.Decode(signedTx.TxBlob).ToObject<Dictionary<string, dynamic>>();
