...
namespace Xahau.Keypairs.Tests
{
    ...
    public class TestUKeypairs
    {
        ...
        {
            string seed = XahauKeypairs.GenerateSeed();
            Assert.AreEqual(seed[0].ToString(), "s");
            DecodedSeed decodedSeed = XahauCodec.DecodeSeed(seed);
        ...
        {
            Assert.AreEqual(XahauKeypairs.GenerateSeed(entropy, "ed25519").ToString(), (string)apiJson["ed25519"]["seed"]);
        }

        [TestMethod]
        public void TestGenerateSeedEDRandom()
        {
            string seed = XahauKeypairs.GenerateSeed(null, "ed25519");
        ...
        {
            IXahauKeyPair keypair = XahauKeypairs.DeriveKeypair((string)apiJson["secp256k1"]["seed"]);
        ...
        {
            IXahauKeyPair keypair = XahauKeypairs.DeriveKeypair((string)apiJson["ed25519"]["seed"]);
        ...
        {
            IXahauKeyPair keypair = XahauKeypairs.DeriveKeypair((string)apiJson["secp256k1"]["seed"], null, true);
        ...
        {
            IXahauKeyPair keypair = XahauKeypairs.DeriveKeypair((string)apiJson["ed25519"]["seed"], null, true);
        ...
        {
            string address = XahauKeypairs.DeriveAddress((string)apiJson["secp256k1"]["keypair"]["publicKey"]);
        ...
        {
            string address = XahauKeypairs.DeriveAddress((string)apiJson["ed25519"]["keypair"]["publicKey"]);
        ...
        public void TestSignSECP()
        {
            ...
            string signature = XahauKeypairs.Sign(messageBytes, privateKey);
        ...
        public void TestVerifySECP()
        {
            ...
            string message = (string)apiJson["secp256k1"]["message"];
            byte[] messageBytes = Encoding.UTF8.GetBytes(message);
            bool verified = XahauKeypairs.Verify(messageBytes, signature, publicKey);
