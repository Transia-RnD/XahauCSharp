...
using Xahau.Keypairs.Ed25519;
using Xahau.Keypairs.K256;
...
{
    public static class XahauKeypairs
    {
        ...
        public static string GenerateSeed(byte[]? entropy = null, string algorithm = "secp256k1")
        {
            ...
            return XahauCodec.EncodeSeed(fentropy, algorithm);
        }
        ...
        public static IXahauKeyPair DeriveKeypair(string seed, string? algorithm = null, bool validator = false, int index = 0)
        {
            DecodedSeed decoded = XahauCodec.DecodeSeed(seed);
            if (decoded.Type == "ed25519")
            {
                IXahauKeyPair edkp = EdKeyPair.From128Seed(decoded.Bytes);
            ...
            IXahauKeyPair scpk = K256KeyGenerator.From128Seed(decoded.Bytes, validator ? -1 : (int)index);
        ...
        public static string DeriveAddressFromBytes(byte[] publicKeyBytes)
            => XahauCodec.EncodeAccountID(Utils.HashUtils.PublicKeyHash(publicKeyBytes));

        public static string DeriveAddress(string publicKey)
            => XahauKeypairs.DeriveAddressFromBytes(publicKey.FromHexToBytes());
    }
}