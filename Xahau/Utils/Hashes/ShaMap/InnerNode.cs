...
namespace Xahau.Utils.Hashes.ShaMap
{
    public class InnerNode : Node
    {
        ...
        public override void AddItem(string tag, Node node)
        {
            ...
            else if (existingNode is LeafNode leaf)
            {
                if (leaf.Tag == tag)
                {
                    throw new XahauException("Tried to add a node to a SHAMap that was already in there.");
                }
                else
                {
        ...
        /// <exception cref="XahauException">If slot is out of range.</exception>
        public void SetNode(int slot, Node node)
        {
            if (slot is < 0 or > SLOT_MAX)
            {
                throw new XahauException("Invalid slot: slot must be between 0-15.");
            }
            Leaves[slot] = node;
            Empty = false;
        }
