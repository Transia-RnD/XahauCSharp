...
using TimeoutException = Xahau.Client.Exceptions.TimeoutException;
...
namespace Xahau.Client
{
    ...
    public class RequestManager
    {
        ...
        public void Resolve(Guid id, BaseResponse response)
        {
            ...
            if (taskInfo == null)
            {
                throw new XahauException($"No existing promise with id {id}");
            }
        ...
        public XahauGRequest CreateGRequest<T, R>(R request, int timeout)
        {
            Guid newId;
            ...
            {
                throw new XahauException($"Response with id '${newId}' is already pending");
            }
        ...
        /// <summary>
        /// </summary>
        public XahauRequest CreateRequest(Dictionary<string, dynamic> request, int timeout)
        {
            Guid newId;
        ...
        public void HandleResponse(BaseResponse response)
        {
            if (response.Id == null)
            {
                throw new XahauException("Valid id not found in response");
            }
            ...
            {
                XahauException error = new XahauException(response.ErrorMessage ?? response.Error);
                this.Reject((Guid)response.Id, error);
                return;
            }
            ...
            {
                XahauException error = new XahauException($"unrecognized response.status: ${response.Status ?? ""}");
                this.Reject((Guid)response.Id, error);
                return;
            }
