...
namespace Xahau.Wallet
{
    public class Signer
    {
        ...
        public static string Multisign(Dictionary<string, dynamic>[] txs)
        {
            ...
            return XahauBinaryCodec.Encode(GetTransactionWithAllSigners(decodedTransactions));
        }
        ...
        public static string AuthorizeChannel(XahauWallet wallet, string channelID, string amount)
        {
            Dictionary<string, dynamic> json = new Dictionary<string, dynamic>();
            json.Add("channel", channelID);
            json.Add("amount", amount);
            string signatureData = XahauBinaryCodec.EncodeForSigningClaim(json);
            return XahauKeypairs.Sign(signatureData.FromHex(), wallet.PrivateKey);
        }
        ...
        public static bool VerifySignature(Dictionary<string, dynamic> tx)
        {
            ...
            return XahauKeypairs.Verify(
              XahauBinaryCodec.EncodeForSigning(decodedTx).FromHex(),
        ...
        {
            string hex = XahauCodec.DecodeAccountID(address).ToHex();
            return new BigInteger(hex, 16);
        }

        public static Dictionary<string, dynamic> GetDecodedTransaction(Dictionary<string, dynamic>  txOrBlob)
        {
            return XahauBinaryCodec.Decode(XahauBinaryCodec.Encode(txOrBlob)).ToObject<Dictionary<string, dynamic>>();
        }

        public static Dictionary<string, dynamic> GetDecodedTransaction(string txOrBlob)
        {
            return XahauBinaryCodec.Decode(txOrBlob).ToObject<Dictionary<string, dynamic>>();
        }
    }
}